  ...
  return {
    ...
    routine: {
      morning: [
        ...
        {id:5,label:"Routine peau matin 🧴 (nettoyant, sérum, hydratant, SPF)",done:false},
        {id:6,label:"Petit-déjeuner sain 🥗",done:false},{id:7,label:"Lire / Écouter 📚",done:false},
      ],
      evening: [
        {id:8,label:"Dîner léger 🥗",done:false},{id:9,label:"30 min de marche 🚶",done:false},
        {id:10,label:"Routine peau soir 🌙 (démaquillage, nettoyant, sérum, crème nuit)",done:false},
        {id:11,label:"Pas d'écran après 21h 📵",done:false},{id:12,label:"Lire 20 min 📖",done:false},
        {id:13,label:"Isha 🕌",done:false},{id:14,label:"Dormir à l'heure 😴",done:false},
      ],
    },
    goals: [],
    finance: {
...
function BottomNav({tab,setTab,onChangePwd}){
      ...
      {showMore&&(
        <div style={{position:"fixed",bottom:70,left:0,right:0,background:C.white,borderTop:`1px solid ${C.border}`,borderRadius:"20px 20px 0 0",padding:"16px 8px 8px",zIndex:200,boxShadow:"0 -8px 32px rgba(78,138,78,0.12)"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4}}>
            ...
            <div onClick={()=>{onChangePwd();setShowMore(false);}} style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"10px 4px",borderRadius:12,cursor:"pointer"}}>
              <span style={{fontSize:22,marginBottom:2}}>🔒</span>
              <span style={{fontFamily:FB,fontSize:10,fontWeight:700,color:C.textDim}}>Mot de passe</span>
            </div>
          </div>
...
// ── Root ──────────────────────────────────────────────────────────────────────
function Dashboard({ onChangePwd }){
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:F,maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      <BottomNav tab={tab} setTab={setTab} onChangePwd={onChangePwd}/>
    </div>
  );
}
...
export default function App(){
  ...
  return <>
    ...
    <Dashboard onChangePwd={() => setShowPwd(true)} />
  </>;
}
